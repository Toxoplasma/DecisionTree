...
class BinaryDecisionFork:
    ...
    def __init__(self, attr, val, numerical, attrname=None, branches=None):
        "Initialize by saying what attribute this node tests."
        update(self, attr=attr, attrname=attrname or attr,
               branches=branches or {}, val=val, numerical=numerical)

    def __call__(self, example):
        "Given an example, classify it using the attribute and the branches."
        attrvalue = example[self.attr]
        if(self.numerical):
            #splitValue = self.parse_numerical(self.val)
            splitValue = self.val
            if(float(attrvalue) <= float(splitValue)):
                return self.branches["<= " + str(splitValue)]
            else:
                return self.branches["> " + str(splitValue)]

        else:
            if(attrvalue == self.val):
                return self.branches[attrvalue](example)
            else:
                return self.branches["Not " + str(self.val)](example)
        #print self.branches
        #return self.branches[attrvalue](example)

    def parse_numerical(self, valueString):
        print valueString
        if(valueString[0] == ">"):
...
def BinaryDecisionTreeLearner(dataset):
    ...
    target, orig_values = dataset.target, dataset.values

    def decision_tree_learning(examples, attrs, values, parent_examples=()):
        if len(examples) == 0:
        ...
        elif check_all_values_empty(attrs, values):
            return plurality_value(examples)
        else:
            A = attrs[0]
            chosen_val = ""
            ...
            if(numerical):
                ...
                array = split_by_binary(A, examples, chosen_val)

            for (v_k, exs) in array:
                subtree = decision_tree_learning(
                    exs, newattrs, values, examples)

                tree.add(v_k, subtree)
            return tree

    def check_all_values_empty(attrs, values):
        for attr in attrs:
            if(values[attr] != []):
                return False
        return True

    def is_numerical(thetype):
        return (thetype is float or thetype is int or thetype is long)
    ...
    def split_by_numerical_without_stuff(attr, value, examples):
        return [(value, [e for e in examples if e[attr] <= value]), 
        (value, [e for e in examples if e[attr] > value])]


    def plurality_value(examples):
        """Return the most popular target value for this set of examples.
        (If target is binary, this is the majority; otherwise plurality.)"""
        popular = argmax_random_tie(orig_values[target],
    ...
    def choose_attribute(attrs, examples, values):
        "Choose the attribute with the highest information gain."
        result = argmax_random_tie(attrs,
                                 lambda a: information_gain(a, examples))
        while values[result] == []:
            attrs = removeall(result, attrs)
            result = argmax_random_tie(attrs,
                                 lambda a: information_gain(a, examples))
        return result
    ...
    def choose_value(attr, examples, values):

        "Chooses the value of an attribute with the most information gain"
        def I(examples):
            return information_content([count(target, v, examples)
        ...
        maxinfo = 0.0
        maxvalue = ""
        attr_type = (type(values[attr][0]))
        split_examples = split_by(attr, examples)
        ...
        for (v, examples_i) in split_examples:
            ...
            if(is_numerical(attr_type)):
                ...
                current_info_gain = I(examples)

                for (vi, examples_iii) in examples_ii:

                    info = (len(examples_iii) / N) * I(examples_iii)

                    current_info_gain -= info

                if current_info_gain >= maxinfo:
                    maxvalue = vi
    ...
    def split_by_binary(attr, examples, chosenValue):
        "Return a list of (val, examples) pairs for one val of attr and the rest."
        #if(values[attr] == []):
        #    values = removall()

        return [(chosenValue, [e for e in examples if e[attr] == chosenValue]), 
        ("Not " + str(chosenValue), [e for e in examples if e[attr] != chosenValue])]

    def split_by(attr, examples):
        "Return a list of (val, examples) pairs for each val of attr."
        return [(v, [e for e in examples if e[attr] == v])
                for v in orig_values[attr]]


    return decision_tree_learning(dataset.examples, dataset.inputs, orig_values, ())
...
irisstrings = DataSet(name="irisstrings", target="class",
               attrnames="sepal-len sepal-width petal-len petal-width class")

smalliris = DataSet(name="smalliris", target="class",
               attrnames="sepal-len sepal-width petal-len petal-width class")

abalone = DataSet(name="abalone", target="Sex", 
                  attrnames="Sex Length Diameter Height Whole Shucked Viscera Shell Rings")

wines =  DataSet(name="wines", target="Region", 
        attrnames="Region Alcohol Malic Ash Alcalinity Magnesium Phenols Flavanoids Nonflavanoidphenols Proanthocyanins Intensity Hue OD280/OD315 Proline")

ecoli = DataSet(name="ecoli", target="Class",
        attrnames = "Sequence mcg gvh lip chg aac alm1 alm2 Class")

servo = DataSet(name="servo", target="motor",
        attrnames = "motor screw pgain vgain class")
